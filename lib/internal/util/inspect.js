...
function parseCssColor(colorString) {
  ...
  if (hashMatch != null) {
    return [
      parseInt(hashMatch[1], 16),
      parseInt(hashMatch[2], 16),
      parseInt(hashMatch[3], 16),
    ];
  }
  const smallHashMatch = RegExpPrototypeExec(SMALL_HASH_PATTERN, colorString);
  if (smallHashMatch != null) {
    return [
      parseInt(`${smallHashMatch[1]}${smallHashMatch[1]}`, 16),
      parseInt(`${smallHashMatch[2]}${smallHashMatch[2]}`, 16),
      parseInt(`${smallHashMatch[3]}${smallHashMatch[3]}`, 16),
    ];
  }
